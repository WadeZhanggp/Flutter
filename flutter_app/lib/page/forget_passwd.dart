...
class _ForgetPasswdPageState extends WdState<ForgetPasswdPage> {
  ...
  void getCode() async {
    ...
    if(countdownTime == 0) {
      startCountdown();
      showLoading();
      var result = await ForgetDao.getVerCode(userName);
      dismissLoading();
  ...
  //注册
  void forgetPasswd() async {
    showLoading();
    var result = await ForgetDao.forget(userName, password, smsCode);
    dismissLoading();
